...
template<typename T>
class Validator {
    public: explicit Validator(std::string componentName) : componentName(std::move(componentName)) {}
    ...
    [[nodiscard]] const std::multimap<std::string, std::string> &getErrors() const {
        return errors;
    }
    ...
    template<typename V> void copyErrorsFrom(const Validator<V> &validator) {
...
protected:
    ...
    void expectAllDataType(const std::string &parameter, const Container &container, const std::string &msg = "") {
     ...
     */
    template<typename U>
    void expectAllInRange(const std::string &parameter, const std::vector<U> &values,
                          const U &min, const U &max, const std::string &msg = "") {
        expectAllInRange(parameter, std::begin(values), std::end(values), min, max, msg);
    }
    ...
    void expectAllInRange(const std::string &parameter, const Iterator begin, const Iterator end,
                          const U &min, const U &max, const std::string &msg = "") {
    ...
    template<typename U>
    void
    expectAllPositive(const std::string &parameter, const std::vector<U> &values) {
        std::set<U> invalidValues;
    ...
    void
    expectDataType(const std::string &parameter, const U &value, const std::string &msg = "") {
        constexpr auto min = (std::numeric_limits<V>::min)();
        constexpr auto max = (std::numeric_limits<V>::max)();
        if(!(value >= min && value <= max)) {
