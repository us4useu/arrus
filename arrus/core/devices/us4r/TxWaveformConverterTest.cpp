...
//             NOTE: the actual number of repetitions is 0 + 2!
        ...
        TxWaveformConverterTestParams{
            ::arrus::ops::us4r::Waveform(
                {
                    WaveformSegment{
                        {
                            0.5e6, // less than max per register
                            129/130e6,  // exactly max
                            (129 + 1)/130e6,  // max + 1 cycle
                            (3*129 + 2)/130e6,  // max + 2 cycles
                            (7*129 + 1)/130e6, // 4*max + 1 max + rest
                            (8*129 + 5)/130e6, // 8*max + 1 max + rest
            ...
            std::vector<uint32_t>{
                ...
                // 8*max + 5 => 4*(2*max) + 5
                0b01010'1111111'1001,
                0b00000'1111111'1001,
                0b00000'0000011'1001,
