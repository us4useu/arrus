...
public:
    Us4OEMUploadResult(Us4OEMBuffer bufferDescription, std::vector<FrameChannelMapping::Handle> fcms, float rxTimeOffset)
        : bufferDescription(std::move(bufferDescription)), fcms(std::move(fcms)), rxTimeOffset(rxTimeOffset) {}
    ...
    float getRxTimeOffset() { return rxTimeOffset; }
private:
