...
TEST_F (Us4OEMTxRxValidatorTest, PreventsTooLongTxDelay) {
    float invalidTxDelay = DEFAULT_DESCRIPTOR.getTxRxSequenceLimits().getTxRx().getTxHV0().getDelay().end();
...
TEST_F (Us4OEMTxRxValidatorTest, PreventsTooShortOrNegativeTxDelay) {
    float invalidTxDelay = DEFAULT_DESCRIPTOR.getTxRxSequenceLimits().getTxRx().getTxHV0().getDelay().start();
    invalidTxDelay -= 1e-6f;
    std::vector<TxRxParameters> txrxs = {
        ARRUS_STRUCT_INIT_LIST(
            TestTxRxParams,
...
TEST_F(Us4OEMTxRxValidatorTest, PreventsInvalidAmplitudeLevel) {
    std::vector<TxRxParameters> txrxs = {
        ARRUS_STRUCT_INIT_LIST(
            TestTxRxParams,
            (x.pulse = Pulse(2e6, 2.0f, false, 3))
                )
            .get()
    };
    TxRxParametersSequence seq = getSequence(txrxs);
...
TEST_F(Us4OEMTxRxValidatorTest, PreventsTooHighFrequency) {
    const auto maxFreq = DEFAULT_DESCRIPTOR.getTxRxSequenceLimits().getTxRx().getTxHV0().getFrequency().end();
...
TEST_F(Us4OEMTxRxValidatorTest, PreventsTooLowFrequency) {
    const auto minFreq = DEFAULT_DESCRIPTOR.getTxRxSequenceLimits().getTxRx().getTxHV0().getFrequency().start();
    std::vector<TxRxParameters> txrxs = {
        ARRUS_STRUCT_INIT_LIST(
            TestTxRxParams,
...
TEST_F(Us4OEMTxRxValidatorTest, PreventsTooLongPulse) {
    const auto maxCycles = DEFAULT_DESCRIPTOR.getTxRxSequenceLimits().getTxRx().getTxHV0().getPulseCycles().end();
