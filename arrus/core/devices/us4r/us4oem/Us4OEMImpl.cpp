...
double Us4OEMImpl::getAfeDemodStartFrequency() { return ius4oem->AfeDemodGetStartFrequency(); }

double Us4OEMImpl::getAfeDemodStopFrequency() { return ius4oem->AfeDemodGetStopFrequency(); }
    ...
    for (uint16 batchIdx = 0; batchIdx < rxBufferSize; ++batchIdx) {
        ...
        for (uint16 seqIdx = 0; seqIdx < batchSize; ++seqIdx) {
            ...
            for (uint16 opIdx = 0; opIdx < seq.size(); ++opIdx) {
                ...
                auto sampleOffset = getNumberOfSamplesToTxStart();
                ius4oem->ScheduleReceive(firing, outputAddress, nSamples, sampleOffset + startSample,
...
unsigned int Us4OEMImpl::getNumberOfSamplesToTxStart() const {
    if (afeDemodEnabled) {
        // AFE IQ demodulator turned on
