...
public:
    ...
    Us4OEMDataTransferRegistrar(Us4ROutputBuffer *dst, const Us4OEMBuffer &src, Us4OEMImplBase *us4oem,
        ...
        this->logger->debug("Us4OEM:{}, transfer strategy: {}", us4oem->getDeviceId().getOrdinal(), strategy);
    }
...
#define ARRUS_ON_NEW_DATA_CALLBACK(signal, strategy)                                                                   \
    [=, this, currentDstIdx = srcIdx, currentTransferIdx = transferIdx]() mutable {                                          \
        ...
        try {                                                                                                          \
            ...
            logger->error("Us4OEM {}: callback exception: {}", us4oemOrdinal, e.what());                               \
        } catch (...) {                                                                                                \
            logger->error("Us4OEM {}: callback unknown exception.", us4oemOrdinal);                                    \
        }                                                                                                              \
    }
