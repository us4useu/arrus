...
float deviceFeedforwardCoefficients[];
extern __device__ __constant__
float deviceFeedbackCoefficients[];
extern __device__ __constant__
float deviceFeedbackCoefficientsMatrix[];
...
void Filter1DGraphNode::readFilterCoefficientsFromTxtFile(const std::string &pathToFile,
                                                          std::vector<float> &filterCoefficients) {
    std::ifstream ifs(pathToFile);
    if(!ifs.good()) {
        DISPATCHER_LOG(DispatcherLogType::ERROR_,
                       std::string("Cannot open file with filter coefficients. File path: ") + pathToFile);
    }
...
void Filter1DGraphNode::readFilterCoefficientsFromBinaryFile(const std::string &pathToFile,
                                                             std::vector<float> &filterCoefficients) {
...
void Filter1DGraphNode::readFilterCoefficientsFromFile(const std::string &pathToFile,
                                                       std::vector<float> &filterCoefficients) {
    ...
    if(found != std::string::npos)
        ...
        if(found != std::string::npos)
            ...
            DISPATCHER_LOG(DispatcherLogType::ERROR_,
                           std::string("File with filter coefficients has wrong extension. File path: ") + pathToFile);
    }
}

std::vector<float>
Filter1DGraphNode::getNormalizedCoefficients(const std::vector<float> &inputCoeff, const float normalizeFactor) {
    std::vector<float> normalizedCoeff(inputCoeff.size());
    std::transform(inputCoeff.begin(), inputCoeff.end(), normalizedCoeff.begin(),
                   [normalizeFactor](const float a) -> float { return a / normalizeFactor; });
    return normalizedCoeff;
}

void Filter1DGraphNode::buildFeedbackCoefficientsMatrix(const std::vector<float> &normFeedbackCoeff,
                                                        cudaStream_t &defaultStream) {
    int feedbackCoefficientsMatrixSize = (int) this->feedbackCoefficients.size() - 1;
    boost::numeric::ublas::matrix<float> feedbackCoefficientsMatrix = boost::numeric::ublas::zero_matrix<float>(
        feedbackCoefficientsMatrixSize, feedbackCoefficientsMatrixSize);
    ...
    for(int i = 0; i < feedbackCoefficientsMatrixSize; ++i)
        feedbackCoefficientsMatrix(i, feedbackCoefficientsMatrixSize - 1) = -normFeedbackCoeff[
            feedbackCoefficientsMatrixSize - i];
    ...
    for(int p = 0; p < IIR_DATA_BLOCK_COUNT - 1; ++p)
        feedbackCoefficientsMatrix = boost::numeric::ublas::prod(feedbackCoefficientsMatrix,
                                                                 initFeedbackCoefficientsMatrix);
    ...
    for(int i = 0; i < feedbackCoefficientsMatrixSize; ++i)
        for(int j = 0; j < feedbackCoefficientsMatrixSize; ++j)
            this->feedbackCoefficientsMatrixAsVector[i * feedbackCoefficientsMatrixSize +
                                                     j] = feedbackCoefficientsMatrix(i, j);
}

void Filter1DGraphNode::loadFilterCoefficients(cudaStream_t &defaultStream) {
    std::string pathToFileWithFeedforwardCoefficients = this->getNodeVariable(
        "feedforwardCoefficients").getValue<std::string>();
    std::string pathToFileWithFeedbackCoefficients = this->getNodeVariable(
        "feedbackCoefficients").getValue<std::string>();
    ...
    if(this->feedbackCoefficientsFileName.compare(pathToFileWithFeedbackCoefficients) != 0) {
        ...
        std::vector<float> normFeedbackCoeff = this->getNormalizedCoefficients(this->feedbackCoefficients,
                                                                               this->feedbackCoefficients[0]);
    ...
    std::vector<float> normFeedbackCoeff = this->getNormalizedCoefficients(this->feedbackCoefficients,
                                                                           this->feedbackCoefficients[0]);
    std::vector<float> normFeedforwardCoeff = this->getNormalizedCoefficients(this->feedforwardCoefficients,
                                                                              this->feedbackCoefficients[0]);
    CUDA_ASSERT(cudaMemcpyToSymbolAsync(deviceFeedbackCoefficients, &normFeedbackCoeff[0],
                                        sizeof(float) * normFeedbackCoeff.size(), 0, cudaMemcpyHostToDevice,
                                        defaultStream));
    if(!normFeedforwardCoeff.empty())
        CUDA_ASSERT(cudaMemcpyToSymbolAsync(deviceFeedforwardCoefficients, &normFeedforwardCoeff[0],
                                            sizeof(float) * normFeedforwardCoeff.size(), 0, cudaMemcpyHostToDevice,
                                            defaultStream));
    if(!this->feedbackCoefficientsMatrixAsVector.empty()) {
        CUDA_ASSERT(
            cudaMemcpyToSymbolAsync(deviceFeedbackCoefficientsMatrix, &this->feedbackCoefficientsMatrixAsVector[0],
                                    sizeof(float) * this->feedbackCoefficientsMatrixAsVector.size(), 0,
...
void Filter1DGraphNode::process(cudaStream_t &defaultStream) {
    ...
    if(!isIIRFilter) {
        if(iqData)
            cudaFilter1DGraphNode.firIq(this->inputData.getPtr<float2 *>(), this->outputData.getPtr<float2 *>(),
                                        defaultStream,
                                        (int) this->feedforwardCoefficients.size(), this->inputData.getDims().flatten(),
                                        batchLength);
        else
            cudaFilter1DGraphNode.fir(this->inputData.getPtr<float *>(), this->outputData.getPtr<float *>(),
                                      defaultStream,
                                      (int) this->feedforwardCoefficients.size(), this->inputData.getDims().flatten(),
                                      batchLength);
    } else {
        if(iqData)
            cudaFilter1DGraphNode.iirIq(this->inputData.getPtr<float2 *>(), this->outputData.getPtr<float2 *>(),
                                        defaultStream,
                                        (int) this->feedforwardCoefficients.size(),
                                        (int) this->feedbackCoefficients.size() - 1,
                                        this->inputData.getDims().flatten(), batchLength);
        else
            cudaFilter1DGraphNode.iir(this->inputData.getPtr<float *>(), this->outputData.getPtr<float *>(),
                                      defaultStream,
                                      (int) this->feedforwardCoefficients.size(),
                                      (int) this->feedbackCoefficients.size() - 1,
                                      this->inputData.getDims().flatten(), batchLength);
