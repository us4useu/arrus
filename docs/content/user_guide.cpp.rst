...
.. note::
    ...
    This functionality is available only for OEM+ systems with firmware version
    greater than ``2.2.0.0``.

For systems with the on-board HVPS (``us4oemhvps``), it is possible to configure
the per-OEM HVPS fuse — i.e. the safety limits on the HVPS output that, when
exceeded, will shut down the high voltage and stop the running TX/RX sequence.
These limits can be specified by adding the ``hvps_fuse`` field to the ``us4r``
settings, for example:

::

    hvps_fuse: {
      level1_max_current_threshold:    0.5    # [A]
      level1_max_power_threshold:      1.35   # [W]
      level1_static_voltage_margin:    2.5    # [V]
      level2_max_current_threshold:    0.125  # [A]
      level2_max_power_threshold:      5.0    # [W]
      level2_static_voltage_margin:    2.5    # [V]
    }

The ``level1`` and ``level2`` qualifiers correspond to the TX pulse/excitation
**amplitude levels** — i.e. the ``amplitude`` attribute of
``arrus::ops::us4r::Pulse`` (values ``1`` and ``2``, see also the **5-level
Waveforms** section). For each amplitude level the following thresholds can be
specified:

- ``level<N>_max_current_threshold``: maximum allowable HVPS output current generated by a single OEM at amplitude level ``N`` [A],
- ``level<N>_max_power_threshold``: maximum allowable HVPS output power generated by a single OEM at amplitude level ``N`` [W],
- ``level<N>_static_voltage_margin``: maximum allowable deviation between the
  measured HVPS output voltage and the requested voltage at amplitude level ``N``
  while no TX/RX is running [V].

All fields are optional — any threshold that is omitted keeps its
**default value, which is determined at run-time** based on the actual hardware
configuration (e.g. HV model, probe voltage range). To rely entirely on the
run-time defaults, simply omit the ``hvps_fuse`` field.
