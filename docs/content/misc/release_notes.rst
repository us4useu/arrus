...
- core (driver):
    ...
    - Reduced logging verbosity (the number of INFO-level entries has been minimized); changed the device ID in the logs from "us4OEM" to "Us4OEM:<ordinal> (<serial>)" #M_US4R-62.
    - Fixed frame metadata timestamps generated by the Us4R system when an external trigger source is used #M_US4R-92.
    - Exposed the possibility to read the HVPS tuning timestamp. #M_OEM-249.
    - Added support for new Interrupt Controller IP address definitions (us4OEM+ firmware 2.2.0.0 only). #M_OEM-207.
    - Fixed handling of single-TX/RX sequences #M_OEM-187.
    - Improved DC/DC voltage setting by fine-tuning for specified HV rail outputs #M_US4R-59.
    - Increased the priority of the TX timeout interrupt and made sure that interrupt 9 is always handled properly (2.2.0.0 firmware only) #M_OEM-233.
...
- core (ARRUS):
    ...
    - Reduced logging verbosity (the number of INFO-level entries has been minimized), increased the default log timestamp precision to milliseconds, and made minor changes to log entry IDs #M_US4R-62.

- C++ API:

    - Exposed HVPS fuse settings in ``Us4RSettings`` (C++ API and ``.prototxt``). #M_OEM-189.
    - Enabled user-defined system-level callbacks, implemented ``Us4RSettingsBuilder``. #M_OEM-286.
    - Exposed system interrupt callbacks; see the user guide for more information. #M_OEM-286.

- Python API:

    - Fixed the processing graph when some Tx/Rx sequence outputs are unused #ARRUS-550.
    - Exposed the possibility to read the HVPS tuning timestamp. #M_OEM-249.
...
- core (driver)
    ...
    - TX timeout protection fixed to work correctly also with corrupted TX_ON signals generated by TX pulsers (generating flat TX delay profiles on using amplitude on level 1) #US4R-570.
    - Fixed the current limitation for the minimum TX frequency (reduced the minimum possible TX frequency from ~504 kHz to 100 kHz) #M_OEM-223.
    - Added TX timeout interrupt handler (the TX/RX sequence will be automatically stopped in case too long TX pulse is detected) #M_OEM-224.
    - Fixed 20 V setting on some OEM+ setups (changed HVPS DCDC PWM for 20V case) #M_OEM-221.
    - Changed voltage accuracy measurement from +/- 4V to +/- 1V #M_OEM-226.
