...
- core (driver):
    ...
    - Implemented HVPS fuse mechanism with over-current, over-power and over-voltage protection using real-time HVPS-based monitoring (2.2.0.0 firmware only). #M_OEM-179, #M_OEM-189, #M_OEM-181, #M_OEM-271.
    ...
    - Linux: rewritten the Linux kernel driver, implemented safe, managed PCIDMA, created .deb package for Linux driver, added LKM signing (secure boot), DKMS support. Updated minimum required Linux kernel version to 5.15. Please update driver to `us4r-lkm-v0.15.0.run` before running firmware update or ARRUS. #M_OEM-240, #M_OEM-241.
    ...
    - Reduced logging verbosity (the number of INFO level entries has been minimized), the device ID us4OEM in the logs has been changed to Us4OEM:ordinal (serial number) #M_US4R-62.
    - Fixed frame metadata timestamps generated by Us4R system when external trigger source is used #M_US4R-92.
    - Exposed possibility to read HVPS tuning timestamp. #M_OEM-249
    - Added support for new Interrupt Controller IP address definitions (us4OEM+ firmware 2.2.0.0 only). #M_OEM-207.
    - Fixed single-TX/RX sequences #M_OEM-187.
    - Improved DC/DC voltage setting by fine-tuning for specified HV rail outputs #M_US4R-59.
    - Increase priority of the TX timeout interrupt, make sure that the interrupt 9 is always properly (2.2.0.0 firmware only) #M_OEM-233.
    - Modified DBARLitePCIe firmware verification to check only the major version. #M_US4R-68.
    - Removed unnecessary components initialization during us4OEM firmware update. #M_OEM-294.
    - Removed soft-start for long TX pulses. #M_US4R-85.
    - *Experimental: enabled GPUDirect RDMA via Linux DMA BUF mechanism #M_OEM-241*.

- core (ARRUS):

    - Reduced logging verbosity (the number of INFO level entries has been minimized), increased the default logging timing precision to milliseconds, minor changes in the log entry IDs #M_US4R-62.
...
- Python API:
   ...
   - Exposed the possibility to read HVPS tuning timestamp.  #M_OEM-249.
   - Enabled user-defined system-level callbacks via ``arrus.create_session_settings_from`` function. #M_OEM-286.
   - Added GPU ``.prototxt`` setting to enable customisation of MemoryPool usage and RAM limit for ``ProcessingRunner``. #ARRUS-543.
