        ...
        function obj = Us4R(varargin)
            ...
            obj.sys.interfEnable = probe.interfEnable;
             ...
             if obj.sys.interfEnable
                 ...
                 obj.sys.xElem = obj.sys.xElem * cos(obj.sys.interfAng) ...
                               + obj.sys.zElem * sin(obj.sys.interfAng);
                 obj.sys.zElem = obj.sys.zElem * cos(obj.sys.interfAng) ...
                               - obj.sys.xElem * sin(obj.sys.interfAng) ...
                               - obj.sys.interfSize;
             end
    ...
    methods(Access = private)
        ...
        function calcTxDelays(obj)
            ...
            if isinf(obj.seq.txFoc)
                ...
                txDelCent	= nan(1,nTx);
                for iTx=1:nTx
                    if ~obj.sys.interfEnable || zFoc(iTx)<0
                        txDel(:,iTx)	= sqrt( (xFoc(iTx) -         xElem.'  ).^2 + ...
                                                (zFoc(iTx) -         zElem.'  ).^2) / sos;	% [s] (nElem x nTx) delays for tx elements
                        txDelCent(iTx)	= sqrt( (xFoc(iTx) - obj.seq.txApCentX).^2 + ...
                                                (zFoc(iTx) - obj.seq.txApCentZ).^2) / sos;	% [s] (1 x nTx) delays for tx aperture center
                    else
                        xRefrElem       = squeeze( refr(single(zElem), ...
                                                        single(xElem), ...
                                                        single(zFoc(iTx)), ...
                                                        single(xFoc(iTx)), ...
                                                        single(obj.sys.interfSos), ...
                                                        single(obj.seq.c), ...
                                                        single(1 / 64 / obj.seq.txFreq)) );
                        xRefrCent       = squeeze( refr(single(obj.seq.txApCentZ), ...
                                                        single(obj.seq.txApCentX), ...
                                                        single(zFoc(iTx)), ...
                                                        single(xFoc(iTx)), ...
                                                        single(obj.sys.interfSos), ...
                                                        single(obj.seq.c), ...
                                                        single(1 / 64 / obj.seq.txFreq)) );
                        % Check what happens when xElem==nan
                        
                        txDel(:,iTx)	= sqrt( (xRefrElem -         xElem.'  ).^2 + ...
                                                (0         -         zElem.'  ).^2) / obj.sys.interfSos ...
                                        + sqrt( (xFoc(iTx) -         xRefrElem).^2 + ...
                                                (zFoc(iTx) -         0        ).^2) / obj.seq.c;	% [s] (nElem x nTx) delays for tx elements
                        txDelCent(iTx)	= sqrt( (xRefrCent - obj.seq.txApCentX).^2 + ...
                                                (0         - obj.seq.txApCentZ).^2) / obj.sys.interfSos ...
                                        + sqrt( (xFoc(iTx) -         xRefrCent).^2 + ...
                                                (zFoc(iTx) -         0        ).^2) / obj.seq.c;	% [s] (1 x nTx) delays for tx aperture center
                    end
                end
