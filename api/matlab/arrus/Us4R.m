...
%                 nChanTot = obj.sys.nChArius*4*nArius;
    ...
    methods(Access = private)
        ...
        function setRecParams(obj,varargin)
            ...
            %% If GPU is available...
            obj.rec.gpuEnable = false; % license('test', 'Distrib_Computing_Toolbox') && ~isempty(ver('distcomp')) && parallel.gpu.GPUDevice.isAvailable;
        ...
        function calcTxRxSubParams(obj)
            ...
            for iArius=0:(nArius-1)
                ...
                % channel mask for a given module and rx aperture
                rxApMaskSelect = obj.seq.rxApMask(obj.sys.selElem(:,iArius+1), :) & obj.sys.actChan(:,iArius+1);
                % use us4OEM physical channels
                nUs4OEMChannels = max(obj.sys.rxChannelMap(iArius+1, :), [], 'all');
                % physical number -> logical number 
                us4oemPhysical2Logical = 1:nUs4OEMChannels;
                us4oemPhysical2Logical(obj.sys.rxChannelMap(iArius+1, :)) = 1:nUs4OEMChannels;
                
                % physical channel -> active channge
                physicalRxSubApMask = rxApMaskSelect(us4oemPhysical2Logical, :);
                physicalRxSubApMask = reshape(physicalRxSubApMask, nChan, 4, nTx);
                % Calculate subapertures
                iSubTx = cumsum(physicalRxSubApMask,2) .* physicalRxSubApMask;
                iSubTx = reshape(iSubTx,[],1,nTx);
                physicalRxSubApMask = reshape(iSubTx == (1:nSubTx),[],nFire);
                % convert back to logical rx aperture
                rxSubApMask(:, :, iArius+1) = physicalRxSubApMask(obj.sys.rxChannelMap(iArius+1, :), :);
        ...
        function img = execReconstr(obj,rfRaw)
            ...
            if strcmp(obj.seq.type,'lin')
                ...
                % B-Mode image reconstruction
                    ...
                    if obj.rec.vectorEnable || obj.rec.colorEnable
                        error("Vector and Color Doppler are currently only supported for GPU.");
                    end
