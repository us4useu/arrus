...
#include "api/matlab/wrappers/devices/probe/convertProbeModelId.h"
#include "arrus/common/asserts.h"
#include "arrus/core/api/devices/probe/ProbeModel.h"
#include "mex.hpp"

namespace arrus::matlab {
::arrus::devices::ProbeModel convertToProbeModel(const MexContext::SharedHandle &ctx,
    ...
    std::vector<ElementIdxType> nElements = getIntVector<ElementIdxType>(ctx, object, "nElements");
    std::vector<double> pitch = getVector<double>(ctx, object, "pitch");
    std::vector<float> frequencyRangeVec = getVector<float>(ctx, object, "txFrequencyRange");
    std::vector<double> voltageRangeVec = getVector<uint8>(ctx, object, "voltageRange");
    ARRUS_REQUIRES_EQUAL(frequencyRangeVec.size(), 2,
                         ::arrus::IllegalArgumentException("Tx frequency range should contain "
                                                           "exactly two elements."));

    return ProbeModel(id, Tuple<ElementIdxType>(nElements), Tuple<double>(pitch),
