...
try
    ...
    nElements = double(probe.nElements);
    ...
    % Define TX impulse. This will be a periodic Pulse with center
    % frequency 5 MHz, 2 periods.
    pulse = Pulse("centerFrequency", 5e6, "nPeriods", 2, "inverse", false);
    ...
    tx = Tx("aperture", txAperture, "delays", zeros(1, nElements), "pulse", pulse);
    rx = Rx("aperture", rxAperture, "sampleRange", [0 nSamples]);
    ops(1) = TxRx("tx", tx, "rx", rx, "pri", 500e-6);

    % Define the second TX/RX 
    delays = linspace(0, 1e-6, nElements);
    tx = Tx("aperture", txAperture, "delays", delays, "pulse", pulse);
    rx = Rx("aperture", rxAperture, "sampleRange", [0 nSamples]);
