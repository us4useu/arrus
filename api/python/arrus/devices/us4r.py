...
class FrameChannelMapping:
    ...
    :param batch_size: the number of RF frames in a single numpy array
    """
    frames: np.ndarray
    channels: np.ndarray
    batch_size: int = 1
...
class Us4R(Device):
    ...
    def upload(self, seq: arrus.ops.Operation, mode="sync",
        ...
        :param rx_batch_size: number of RF frames that should be acquired in
          a single run
        ...
        if host_buffer_size % rx_batch_size != 0:
            raise ValueError("Host buffer size should be a multiple of "
                             "rx batch size.")
        ...
        return HostBuffer(
            ...
            frame_shape=self._get_physical_frame_shape(
                fcm, n_samples, rx_batch_size=rx_batch_size),
    ...
    def _get_physical_frame_shape(self, fcm, n_samples, n_channels=32,
        ...
        # This might not be the case in the future improvements.
        n_frames = np.max(fcm.frames) + 1
        # Number of frames for a single batch element
