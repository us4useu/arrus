        ...
        stage('Publish docs') {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: '00c79f7e-f299-4ec6-959d-9d09785891f5', usernameVariable: 'username', passwordVariable: 'password')]){
                    sh 'python ${env.WORKSPACE}/scripts/publish_docs.py' --install_dir='${env.ARRUS_INSTALL_DIR}/${env.BRANCH_NAME}' --repository "https://$username:$password@github.com/us4useu/arrus-public.git" --src_branch_name "develop" --build_id ${env.BUILD_ID}
                }
            }
        }
    }
