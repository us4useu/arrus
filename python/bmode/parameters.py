...
def load_mat_file(mat_file):
    ...
    return tuple(_load_mat_structure(attr_class, mat_file[attr_name])
                 for attr_name, attr_class in MATLAB_ROOT_STRUCTURES)



def _load_mat_structure(py_class, mat_structure):
    ...
    for attr, attr_class in py_class_attrs.items():
        ...
        if issubclass(attr_class, BmodeDescriptor):
            value = _load_mat_structure(attr_class, mat_value)
            ...
            if type(value) != attr_class:
                if np.can_cast(type(value), attr_class):
