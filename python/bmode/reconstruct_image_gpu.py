...
def reconstruct_rf_img(rf, x_grid, z_grid,
    ...
    element_xcoord=cp.linspace(-probe_width/2, probe_width/2, n_channels)
    ...
    tail=cp.zeros((1, n_channels, n_transmissions))
    rf=cp.concatenate((rf, tail))

    # buffers allocation
    rf_tx=cp.zeros((z_size, x_size, n_transmissions))
    if is_iqdata:
        rf_tx=rf_tx.astype(complex)

    weight_tx=cp.zeros((z_size, x_size, n_transmissions))
    ...
    for itx in range(0, n_transmissions):
        ...
        if tx_mode=='lin':
            ...
            xdifference=cp.array(x_grid-element_xcoord[itx])
            ...
            tx_distance=cp.tile(z_grid, (1, int(n_valid)))
            #             tx_distance = cp.tile(z_grid, 3)
            tx_apodization=cp.ones((z_size, int(n_valid)))
            # TODO: Should be better image close to the transducer (apodization)

        # synthetic transmit aperture method
        elif tx_mode=='sta':
            lix_valid=cp.ones(x_size, dtype=bool)
            tx_distance=cp.sqrt((z_grid-tx_focus)**2
                                +(x_grid-element_xcoord[itx])**2
                                )

            tx_distance=tx_distance*cp.sign(z_grid-tx_focus)+tx_focus

            f_number=max(cp.concatenate(abs(z_grid-tx_focus), 1e-12)) \
            ...
            tx_distance= \
                (x_grid-element_xcoord[first_element])*cp.sin(tx_angle[itx]) \
                +z_grid*cp.cos(tx_angle[itx])

            r1=(x_grid-element_xcoord[0])*cp.cos(tx_angle[itx]) \
               -z_grid*cp.sin(tx_angle[itx])

            r2=(x_grid-element_xcoord[-1])*cp.cos(tx_angle[itx]) \
               -z_grid*cp.sin(tx_angle[itx])
            ...
            # buffers allocation
        rf_rx=cp.zeros((z_size, x_size, n_channels))
        if is_iqdata:
            rf_rx=rf_rx.astype(complex)

        weight_rx=cp.zeros((z_size, x_size, n_channels))

        # loop over elements
        for irx in range(0, n_channels):

            # calculate rx delays and apodization
            rx_distance=cp.sqrt((x_grid[lix_valid]-element_xcoord[irx])**2
                                +z_grid**2)
            ...
            valid=np.where(lix_valid)[0].tolist()
            rf_rx[:, valid, irx]=rf_raw_line[floor_samples]*(1-(samples%1)) \
                                 +rf_raw_line[ceil_samples]*(samples%1)

            #             floor_line_samples = rf_raw_line[floor_samples.flatten()].reshape((z_size, x_size))
            #             ceil_line_samples = rf_raw_line[ceil_samples.flatten()].reshape((z_size, x_size))
            # print(rf_rx[:, lix_valid.tolist(), irx])
            # channel was lix_valid
            #             rf_rx[:, :, irx] = floor_line_samples*(1 - (samples % 1))\
            #                                        + ceil_line_samples*(samples % 1)

            #             weight_rx[:, lix_valid, irx] = tx_apodization * rx_apodization

            #             print(weight_rx.shape)

            apo_is_nonempty=tx_apodization.size!=0 and rx_apodization.size!=0
            ...
            # channel was lix_valid
            #             weight_rx[:, :, irx] = tx_apodization * rx_apodization
            ...
            if is_iqdata:
                ...
                rf_rx[:, lix_valid, irx]=rf_rx[:, lix_valid, irx] \
                                         *cp.exp(1j*2*np.pi*fc*delays)
                pass

        # calculate rf and weights for single tx
        rf_tx[:, :, itx]=cp.sum(rf_rx*weight_rx, axis=2)
        weight_tx[:, :, itx]=cp.sum(weight_rx, axis=2)
    ...
    rf_image=cp.sum(rf_tx, axis=2)/cp.sum(weight_tx, axis=2)
...
# x_grid=np.linspace(-20*1e-3, 20*1e-3, 256)
# z_grid=np.linspace(0.5*1e-3, 50.*1e-3, 1024)
#
# # reconstruct data
